...
static inline int depack(void *src, void *dst, int srcsize) {
  ...
  case 1:
  case 5:   // sixpack
    // return sixdepak(src, dst, srcsize);
    break;
  case 2:
  case 6:   // FIXME: lzw
    break;
...
const unsigned int Ca2mLoader::MAXFREQ = 2000,
                   ...
                   Ca2mLoader::ROOT = 1, 
                   Ca2mLoader::MAXBUF = 42 * 1024,
                   Ca2mLoader::MAXDISTANCE = 21389, 
                   Ca2mLoader::MAXSIZE = 21389 + MAXCOPY;
...
bool Ca2mLoader::load(const std::string &filename, const CFileProvider &fp) {
  ...
  unsigned short len[17], *secdata, *secptr;
  ...
  if (strncmp(id, "_A2module_", 10) || (version != 1 && version != 5 && version != 4 && version != 8 && version != 9)) {
    fp.close(f);
  ...
  memcpy(instname, orgptr, instcount * 33); 
  orgptr += instcount * 33;
  ...
  // blocks 1-4 or 1-8
  alength = len[1];
  int stop_at = version < 5 ? numpats / 16 : numpats / 8;
  ...
  if (version == 1 || version == 5 || version > 8) {
    ...
    if (version < 9) {
      ...
      orgptr += depack(secptr, orgptr, len[1]); secptr += len[1] / 2;
    }
    ...
    if (version == 1) {
      ...
      if (numpats > 8)
        orgptr += depack(secptr, orgptr, len[2]); secptr += len[2] / 2;

      if (numpats > 16)
        orgptr += depack(secptr, orgptr, len[3]); secptr += len[3] / 2;

      if (numpats > 24)
        orgptr += depack(secptr, orgptr, len[4]); secptr += len[4] / 2;

      if (numpats > 32)
        orgptr += depack(secptr, orgptr, len[5]); secptr += len[5] / 2;

      if (numpats > 40)
        orgptr += depack(secptr, orgptr, len[6]); secptr += len[6] / 2;

      if (numpats > 48)
        orgptr += depack(secptr, orgptr, len[7]); secptr += len[7] / 2;

      if (numpats > 56)
        depack(secptr, orgptr, len[8]);
    }
