  ...
  if (version < 5) {
    ...
    for (i = 0; i < numpats; i++) {
      for (j = 0; j < 18; j++) {
        for (k = 0; k < 64; k++) {
          track_index = i * 18 + j;
          struct Tracks *track = &tracks[track_index][k];
          ...
          if (track->note > 0) {
            printf("[/%d] p: %d, c: %d, r: %d, n: %d, inst: %d\n", track_index, i, j, k, track->note, track->inst);
          }
    ...
    for (i = 0; i < numpats; i++) {
      for (j = 0; j < ntracks; j++) { // channel
        for (k = 0; k < pattern_len; k++) { // row
          ...
          if (track->note > 0) {
            printf("p: %d, c: %d, r: %d, n: %d, inst: %d\n", i, j, k, track->note, track->inst);
          }
